...
class Q2Agent(ReinforcementAgent):
    ...
    def getQValue(self, state: tuple, action):
        ...
        """
        "*** YOUR CODE HERE ***"
        x, y = state  # Extract the x and y coordinates from the state tuple
        action_index = self.getActionIndex(action)  # Get the action index using the provided method
        return self.Q_values[x][y][action_index]  # Return the Q-value from the Q-table
    ...
    def computeValueFromQValues(self, state: GameState):
        ...
        if not legal_actions:
            return 0.0  # Return 0.0 if there are no legal actions
        x, y = state.getPacmanPosition()  # Get Pacman's current position
        q_values = [self.getQValue((x, y), action) for action in legal_actions]  # Get Q-values for all legal actions
        return max(q_values)  # Return the maximum Q-value
    ...
    def epsilonGreedyActionSelection(self, state: GameState):
        ...
        if util.flipCoin(self.epsilon):
            return random.choice(legal_actions)  # Explore: choose a random action
        else:
    ...
    def update(self, state: GameState, action, nextState: GameState, reward):
        ...
        # Compute the sample, which is the target Q-value
        legal_actions_next = self.getLegalActions(nextState)
        if not legal_actions_next:
            max_q_next = 0.0  # If terminal state, the future Q-value is 0
        else:
        ...
        old_q_value = self.Q_values[x][y][action_index]
        self.Q_values[x][y][action_index] = old_q_value + self.alpha * (sample - old_q_value)
